...
from adapter_modules import InteractionBlockWithCls
from adapter_modules import deform_inputs
from clip_vit_hf import CLIPVisionEmbeddings
from ops.modules import MSDeformAttn
...
if __name__ == "__main__":
    visual_tokenizer = clip_vit_adapter_hf(model_path="/liymai24/sjtu/bokai/MM-Interleaved/assets/openai/clip-vit-large-patch14").cuda()
    image = torch.randn(8, 3, 336, 336).cuda()
    model_output = visual_tokenizer(image)
    print(model_output.last_hidden_state.shape)                     # torch.Size([8, 257, 1024])
    print(model_output.pooler_output.shape)                         # torch.Size([8, 1, 1024])
    
# torch.Size([8, 1024, 64, 64])
# torch.Size([8, 1024, 32, 32])
# torch.Size([8, 1024, 16, 16])
# torch.Size([8, 1024, 8, 8])
    for item in model_output.hidden_states:             
        print(item.shape)
    # return BaseModelOutputWithPooling(
    #         last_hidden_state=last_hidden_state,
    #         pooler_output=pooled_output,
...
# torch.Size([8, 577, 1024])
# torch.Size([8, 1, 1024])
# torch.Size([8, 1024, 96, 96])
# torch.Size([8, 1024, 48, 48])
# torch.Size([8, 1024, 24, 24])
